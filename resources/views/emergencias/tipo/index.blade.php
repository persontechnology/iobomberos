...
<div class="card">
    <div class="card-header">
        <form action="{{ route('guardarTipoEmergencia') }}" method="POST" id="formGuardar">
            @csrf
            <input type="hidden" name="emergencia" value="{{ $emergencia->id }}">
            <label for="nombre">Nombre de tipo emergencia</label>
    ...
    <script>
        ...
        function eliminar(arg){
            ...
            function(){
                ...
                $.post( "{{ route('eliminarTipoEmergencia') }}", { teme: $(arg).data('id') })
                .done(function( data ) {
                    if(data.success){
